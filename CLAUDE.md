...
**Eliminazione multipla**: mai subito dalla `BulkBar`. `ConfirmDialog` col conteggio, selezione controllata (annulla la rimette), esito per numero con i rifiuti per vincolo (23503) a parte. In Prodotti è `useBulkDelete` (`src/pages/Dashboard/Products/hooks/`); Menù e Programmazione hanno la stessa forma scritta in pagina.
