...
- **Dettaglio sede** (`/business/:businessId/locations/:activityId`) — 7 tab via `?tab=` (profile/hours/sala/availability/ordering/reservations/settings; `tables` legacy → `sala`). Valori+label in `TAB_VALUES`/`TAB_LABELS`. FASE 6 in corso: `hours`/`ordering`/`reservations` montano ancora `ActivitySettingsTab` intera. Dettaglio: `docs/patterns/activity-detail.md`.
