...
export default function PublicSheet({ isOpen, onClose, children, ariaLabel, headerContent, footerContent, contentKey }: Props) {
    ...
    // ── Body lock — meccanica neutra, estratta in hook ──────────────────────
    // Il lock/unlock del body (con l'ordine atomico iOS e il contatore sheet
    // aperte) non dipende dall'ancoraggio della superficie: vive in
    // useSheetBodyLock. Il QUANDO del rilascio resta qui — animateOutMobile
    // chiama releaseBodyLock() a inizio uscita, lockBody() ri-blocca sulla
    // close-interruption.
    const { releaseBodyLock, lockBody } = useSheetBodyLock(isOpen);
