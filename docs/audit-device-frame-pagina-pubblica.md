...
## FASE 3.4 — Esito (barra di controllo + appartenenza tenant)

- **Gap cross-tenant chiuso** (Notion, priorità ALTA, area Privacy e Sicurezza). `resolve-public-catalog` applica `?simulate=` solo se il chiamante ha una relazione reale con il tenant della sede risolta: client user-scoped (anon key + JWT) → RPC `get_my_tenant_ids()` (owner o membership attiva, qualunque ruolo). Helper `_shared/tenantMembership.ts` (`extractBearerJwt`, `parseTenantIds`, `isTenantMember`), fail-closed, senza import Deno → testato con vitest. "Autenticato da qualche parte" non basta più.
