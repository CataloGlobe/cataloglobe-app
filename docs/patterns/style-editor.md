...
- `navigationStyle` valori correnti: `"filled" | "outline" | "tabs" | "minimal"`. I valori deprecati `"pill"`, `"chip"` e `"dot"` (variante rimossa) sono rimappati a `"filled"` in `parseTokens` — la label UI nel PropertiesPanel resta "Pill" per familiarità.
