...
- **RPC `analytics_*` in SECURITY INVOKER (eccezione voluta)**: le RPC analytics (engagement, ordini, prenotazioni) usano **SECURITY INVOKER + RLS**, non il pattern SECURITY DEFINER delle altre RPC sensibili. Motivo: le tabelle sottostanti (`orders`, `reservations`, `analytics_events`) hanno già RLS via `has_permission('orders.read'|'reservations.read', activity_id)`, quindi l'isolamento è garantito dal chiamante. Nessun GRANT/REVOKE/`search_path` custom. NON convertire a DEFINER per uniformità: romperebbe il filtro per-permesso.
