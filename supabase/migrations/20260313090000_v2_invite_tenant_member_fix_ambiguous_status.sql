...
-- =========================================
-- V2: FIX AMBIGUOUS COLUMN REFERENCE IN invite_tenant_member
-- =========================================
--
-- PostgreSQL raises 42702 (ambiguous column) because the function's output
-- TABLE has columns named `status` and `role`, which shadow the identically
-- named columns of v2_tenant_memberships inside the authorization guard
-- SELECT.  Adding the table alias `tm` and qualifying every column reference
-- in that query resolves the ambiguity.
--
-- No logic changes — only the alias + qualification are added.
-- =========================================
...
BEGIN
  ...
  -- Table alias `tm` added to avoid ambiguity with the output column `status`.
  IF NOT EXISTS (
    SELECT 1
