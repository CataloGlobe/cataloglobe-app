-- Fix-forward: v_idem_inserted must be INTEGER, not BOOLEAN.
-- `GET DIAGNOSTICS v_idem_inserted = ROW_COUNT` assigns an integer; the later
-- `IF v_idem_inserted = 0` then evaluated `boolean = integer` → SQLSTATE 42883
-- "operator does not exist: boolean = integer", failing EVERY submit carrying an
-- idempotency_key (the edge always sends one) → 100% of orders would 500.
-- CREATE OR REPLACE on the same 11-arg signature preserves the existing
-- service_role-only grants, so no companion grant migration is needed.
