-- =========================================
-- Public API: extend get_public_catalog()
-- - add featured_contents
-- - add pricing (base vs override) per product
-- =========================================
...
BEGIN
  ...
  v_payload :=
    jsonb_build_object(
      ...
      -- Featured contents (tenant-scoped via catalog.tenant_id)
      'featured_contents', COALESCE(
        (
          SELECT jsonb_agg(
            ...
            ORDER BY COALESCE(fc.sort_order, 0), fc.created_at
          )
      ...
      -- Categories with products + pricing
      'categories', COALESCE(
        (
          SELECT jsonb_agg(
            jsonb_build_object(
                ...
                (
                  SELECT jsonb_agg(
                    jsonb_build_object(
                      ...
                      'product', to_jsonb(p),
                      'pricing', (
                        -- pricing object (override most recent enabled schedule)
