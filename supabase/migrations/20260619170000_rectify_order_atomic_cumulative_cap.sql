-- ============================================================
-- rectify_order_atomic — enforce a CUMULATIVE per-line residual cap and
-- write row->row lineage (parent_order_item_id) on each storno line.
--
-- Re-emits the live function VERBATIM via CREATE OR REPLACE, changing ONLY:
--   (1) Section 5 INSERT now sets order_items.parent_order_item_id = the
--       parent order_item id being stornato.
--   (2) Section 3 now aggregates the requested storno quantities per parent
--       order_item (dedup within the payload) and validates each against the
--       RESIDUAL stornabile = original_qty - already_stornato (sum of prior,
--       non-cancelled rectifications of this parent keyed by
--       parent_order_item_id). The old per-call check
--       (STORNO_QTY_EXCEEDS_ORIGINAL) is SUBSUMED by the residual check
--       (residual <= original) and is replaced by STORNO_QTY_EXCEEDS_RESIDUAL.
--
-- Everything else (params, SECURITY DEFINER, search_path='', other
-- validations, order creation, total_amount, parent field inheritance) is
-- unchanged. CREATE OR REPLACE preserves existing GRANT/REVOKE.
-- ============================================================
