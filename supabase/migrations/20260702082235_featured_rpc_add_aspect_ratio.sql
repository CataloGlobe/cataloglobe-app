-- Add media_aspect_ratio to the featured_content JSONB returned by
-- get_schedule_featured_contents(uuid, uuid).
--
-- Column added in 20260702081221_add_featured_media_aspect_ratio.sql.
--
-- Additive: one extra key inside featured_content. Row set, tenant guard
-- (p_tenant_id binding), published filter, product join, ordering, SECURITY
-- DEFINER and grants are preserved verbatim from 20260701205352.
    ...
    jsonb_agg(
      jsonb_build_object(
        ...
        'featured_content', jsonb_build_object(
          ...
          'media_aspect_ratio',  fc.media_aspect_ratio,
