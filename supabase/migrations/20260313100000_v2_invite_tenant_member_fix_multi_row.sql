...
-- =========================================
-- V2: FIX NON-DETERMINISTIC SELECT INTO IN invite_tenant_member
-- =========================================
--
-- Three SELECT … INTO queries could raise "query returned more than one row"
-- when a tenant/user pair has accumulated multiple historical membership rows
-- (e.g. expired then revoked then re-invited).
--
-- Fix: add ORDER BY tm.created_at DESC LIMIT 1 to each of the three queries
-- so PostgreSQL always picks the most-recent row deterministically.
--
-- Affected queries:
--   1. Path A — user_id membership lookup
--   2. Path A — stale email-only invite check (v_status only)
--   3. Path B — email-only membership lookup
--
...
BEGIN
  ...
  IF v_user_id IS NOT NULL THEN
    ...
    -- FIX 1: ORDER BY + LIMIT 1 to avoid "query returned more than one row"
    ...
    IF v_existing_id IS NULL THEN
      -- FIX 2: ORDER BY + LIMIT 1 to avoid "query returned more than one row"
  ...
  ELSE
    ...
    -- FIX 3: ORDER BY + LIMIT 1 to avoid "query returned more than one row"
    SELECT tm.id, tm.status INTO v_existing_id, v_status
    FROM public.v2_tenant_memberships tm
    WHERE tm.tenant_id            = p_tenant_id
      AND lower(tm.invited_email) = p_email
