-- =========================================
-- Schedules: aggiungi priority_level + display_order
-- Correggi bug ORDER BY priority DESC → ASC nelle RPC pubbliche
-- =========================================

BEGIN;

-- 1. Aggiungi enum
CREATE TYPE schedule_priority_level AS ENUM ('low', 'medium', 'high', 'urgent');

-- 2. Aggiungi colonne
ALTER TABLE schedules
  ADD COLUMN priority_level schedule_priority_level NOT NULL DEFAULT 'medium',
  ADD COLUMN display_order integer NOT NULL DEFAULT 0;

-- 3. Migra valori esistenti da priority a priority_level
UPDATE schedules SET priority_level = CASE
  WHEN priority <= 2  THEN 'urgent'::schedule_priority_level
  WHEN priority <= 5  THEN 'high'::schedule_priority_level
  WHEN priority <= 8  THEN 'medium'::schedule_priority_level
  ELSE                     'low'::schedule_priority_level
END;

-- 4. Correggi bug: ORDER BY s.priority DESC → ASC in get_public_catalog()
-- La versione corretta è basata su 20260228175000 con priority ASC
