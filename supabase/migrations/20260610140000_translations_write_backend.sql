-- Tighten translations backend to enforce has_permission('translations.write').
--
-- Before: any tenant member could activate/deactivate languages and retry
-- translations (only membership was checked, or not at all).
-- After:  requires translations.write (owner/admin only).
--
-- Changes:
--   1. tenant_languages INSERT/UPDATE/DELETE policies → add has_permission check
--   2. retry_all_failed_translations RPC → replace membership check with
--      has_permission('translations.write') (covers owner synthetic row too)
--   3. enqueue_tenant_language_backfill RPC → add has_permission check
...
-- ── 3. enqueue_tenant_language_backfill ──────────────────────────────────────
