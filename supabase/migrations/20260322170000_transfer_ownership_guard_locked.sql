...
-- =============================================================================
-- RPC: transfer_ownership — guard against locked/deleted tenant
-- =============================================================================
--
-- Adds Guard 4 after the existing tenant-row lock:
--   If tenants.locked_at IS NOT NULL → RAISE (tenant is undergoing deletion)
--   If tenants.deleted_at IS NOT NULL → RAISE (tenant is soft-deleted)
--
-- Why here (after FOR UPDATE, before membership updates):
--   The existing PERFORM already acquires FOR UPDATE on the tenants row,
--   serializing any concurrent operation that also targets the same row.
--   By promoting PERFORM to SELECT ... INTO, we read the authoritative state
--   of locked_at / deleted_at under that lock — eliminating the race window
--   between execute_account_deletion_tenant_ops() setting locked_at and
--   transfer_ownership() proceeding with membership updates.
--
--   Timeline before this fix:
--     T1: execute_account_deletion_tenant_ops → SET locked_at = now()
--     T2: transfer_ownership → PERFORM FOR UPDATE (no state read)
--     T2: transfer_ownership → proceeds, changes owner while tenant is locked
--   Result: tenant locked + ownership changed = incoherent state.
--
--   Timeline after this fix:
--     T1: execute_account_deletion_tenant_ops → acquires FOR UPDATE, SET locked_at
--     T2: transfer_ownership → acquires FOR UPDATE (waits for T1 to commit)
--     T2: reads locked_at = now() → RAISE tenant_locked → rollback
--   Result: transfer rejected cleanly, no state mutation.
--
...
BEGIN
  ...
  -- authoritative state.
  --
  -- Promoted from PERFORM to SELECT ... INTO so that locked_at and
  -- deleted_at are captured under the same lock, closing the race
  -- window with execute_account_deletion_tenant_ops().
  -- ------------------------------------------------------------------
  ...
  --
  -- locked_at is set by execute_account_deletion_tenant_ops() when the
  -- owner chooses to lock rather than transfer. A locked tenant must
  -- not change ownership — the lock is part of the account deletion
  -- flow and must remain consistent with owner_user_id for recovery
  -- (unlock_owned_tenants) to work correctly.
  -- ------------------------------------------------------------------
